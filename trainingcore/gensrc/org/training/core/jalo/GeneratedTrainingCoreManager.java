...
import org.training.core.jalo.TrainingApparelProduct;
...
public abstract class GeneratedTrainingCoreManager extends Extension
{
	...
	public TrainingApparelProduct createTrainingApparelProduct(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.TRAININGAPPARELPRODUCT );
			return (TrainingApparelProduct)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating TrainingApparelProduct : "+e.getMessage(), 0 );
		}
	}
	
	public TrainingApparelProduct createTrainingApparelProduct(final Map attributeValues)
	{
		return createTrainingApparelProduct( getSession().getSessionContext(), attributeValues );
	}
